...
public class JobApplyBasicInfoDao {
    private final int LIMIT = 3;
    /* 将求职信息写入数据库 */
    public int writeJobApplyInfoToSql(Object...args){
        String sql = "insert into jobapply_basicinfo (applicant_id,jobApplyType,jobApplySalary,jobApplyForAddress) values (?,?,?,?)";
...
//
    ...
    /* 获取指定行的数据 */
    public List<JobApplyBasicInfo> findAll(Integer page) throws SQLException, IOException, ClassNotFoundException {
        String sql = "select * from jobapply_basicinfo limit ?,?";
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet resultSet = null;
        List<JobApplyBasicInfo> books = new ArrayList();
        try {
            // 获取连接
            connection = GetSqlConnection.getConnection();
            // 预编译sql语句
            ...
            while (resultSet.next()){
                books.add(new JobApplyBasicInfo(resultSet.getInt(1),resultSet.getInt(2),resultSet.getString(3),resultSet.getString(4),resultSet.getString(5)));
    ...
    /* 获取表中记录的总个数 */
    public int getPages(){
        String sql = "select count(*) from jobapply_basicinfo";
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet resultSet = null;
        int count = 0;
    ...
    public List<RecruitBasicInfo> find(Object...args) throws SQLException, IOException, ClassNotFoundException {
        String sql = "select * from jobapply_basicinfo where jobName like ? and jobType like ? and workAddress like ? ";
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet resultSet = null;
        List<RecruitBasicInfo> books = new ArrayList();
